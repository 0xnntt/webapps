...
def fetch_screen(protocol, hostname, port, output, logfile, useragent_abr, keep, storage):
    b = screenshot(
            protocol=protocol,
            hostname=hostname,
            port=port,
            ...
            logfile=logfile,
            output=output,
            useragent_abr= useragent_abr
            )
    b.capture(storage=storage)

class project:
    """ This class will define every project with its setting"""
    def __init__(self, input_file, output_folder, port, protocol, eachsteps, dbtemplate, threads, useragent , save_to_db=False):
    ...
    def start(self):
        ...
        try:
            if not os.path.exists(os.path.join(self.output, self.name)):
                os.umask(0000)
                os.makedirs(os.path.join(self.output, self.name,"screenshots"))
                        ...
                        try:
                            ...
                            for i in self.steps_data:
                                for j in tmp_screenshot_data:
                                    if(i['url'] == j['url']):
                                        i['data']['image_path'] = j['image_path']
                                        ...
                                        #pdb.set_trace()
                                        img_relative_path = i['data']['image_path'].split(self.name)[-1][1:] #[1:] for removing first / for correct format in os.path.join
                                        c.executemany(
                                        "INSERT INTO presentation_urldata (protocol,hostname,port,has_ssl,useragent,ctime,date,status_code,status,content,http_header,certificate,image_path,screenshot_date,project_id,author_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                            [
                                                (i['data']['protocol'],
                                                 i['data']['hostname'],
                                                 i['data']['port'],
                                                 ...
                                                 i['data']['headers'],
                                                 i['data']['certificate'],
                                                 os.path.join(self.name, img_relative_path ),
